#include "BTService_UpdateSelfStats.h"
...
UBTService_UpdateSelfStats::UBTService_UpdateSelfStats()
{
	NodeName = TEXT("Update Self Stats");
	Interval = 0.2f;
	RandomDeviation = 0.05f;
	HealthPctKey.AddFloatFilter( this, GET_MEMBER_NAME_CHECKED( UBTService_UpdateSelfStats, HealthPctKey ) );
	StaminaPctKey.AddFloatFilter( this, GET_MEMBER_NAME_CHECKED( UBTService_UpdateSelfStats, StaminaPctKey ) );
}

void UBTService_UpdateSelfStats::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
